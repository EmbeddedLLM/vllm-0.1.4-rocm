...
if ! patch -R -p0 -s -f --dry-run $XFORMERS_FMHA_FLASH_PATH "flashpy_xformers-0.0.22.post7.patch"; then
    echo "Applying patch to ${XFORMERS_FMHA_FLASH_PATH}"
    patch -p0 $XFORMERS_FMHA_FLASH_PATH "flashpy_xformers-0.0.22.post7.patch"
    echo "Successfully patch ${XFORMERS_FMHA_FLASH_PATH}"
else
    echo "${XFORMERS_FMHA_FLASH_PATH} was patched before"
fi

if ! patch -R -p0 -s -f --dry-run $XFORMERS_FMHA_COMMON_PATH "commonpy_xformers-0.0.22.post7.patch"; then
    echo "Applying patch to ${XFORMERS_FMHA_COMMON_PATH}"
    patch -p0 $XFORMERS_FMHA_COMMON_PATH "commonpy_xformers-0.0.22.post7.patch"
